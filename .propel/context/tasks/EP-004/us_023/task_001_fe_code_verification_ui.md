...
- [x] Create `useCodeSuggestions.ts` (60s staleTime) and `useConfirmCode.ts` (optimistic `setQueryData` on success)
- [x] Create `CodeSuggestionCard.tsx`: ICD-10 badge purple (`#673AB7`), CPT badge teal (`#009688`) from `FACT_CATEGORY_COLORS`; evidence `Chip` array; Accept (green, disabled when reject-field active) / Reject (expand inline `TextField` + Confirm + Cancel) / Modify; accepted/rejected completed state with colour-coded `Chip`; validation: empty justification → `justErr=true`, mutation NOT called
- [x] Create `CodeVerificationPage.tsx` (SCR-019): breadcrumb UXR-002; responsive `code-grid` (`minmax(400px, 1fr)` → 1-col at xs); Loading (Skeleton ×3) / Empty (Alert info) / Error (Alert+Retry) states; fixed bottom bar with `Tooltip` finalize gate; reuse `SourceCitationDrawer` + `useFactSource` on evidence chip click; call `PATCH /api/v1/360-view/{id}/status = verified` → navigate to SCR-020 with `{patientName, factCount, codesConfirmed, startedAt, avgConfidence}` in location state
- [x] Create `VerificationCompletePage.tsx` (SCR-020): centred card (max-width 600px); 80px green circle check icon; `Alert success` chart summary; 2-col stats grid (time to verify computed from `location.state.startedAt`, AI confidence avg); "Next patient" → `/staff/patients`
- [x] Register both routes in `App.tsx` inside `<StaffRouteGuard>`: `staff/patients/:patientId/code-verification` + `staff/patients/:patientId/verification-complete`
- [x] **[UI Tasks - MANDATORY]** Reference `wireframe-SCR-019-code-verification.html` and `wireframe-SCR-020-verification-complete.html` from Design References table during implementation — verified: grid `minmax(400px,1fr)`, card `border: 1px solid #E0E0E0 + borderRadius: 8px + boxShadow 1`, badge colours, breadcrumb-chip hover `bgcolor: primary.main`, bottom bar fixed, completion card 80px icon, `border-left: 4px solid #4CAF50`, 2-col stats, "Next patient" CTA
- [x] **[UI Tasks - MANDATORY]** Validate UI matches both wireframes before marking task complete — implemented: all layout regions, states (loading/empty/error/default/reviewed), badge colours, fixed bottom bar, completion card exactly match wireframe spec; also created `client/src/api/codeSuggestions.ts` with domain types and API functions
